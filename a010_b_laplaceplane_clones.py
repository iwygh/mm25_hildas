...
objnames = ['Potomac','Hilda','Schubart','Ismene']
nnames = len(objnames)
for iname in range(nnames):
    objname = objnames[iname]
    aau_mat = np.loadtxt('b005_'+objname+'_aau_clones_'+yrsstr+'.csv',delimiter=',')
    nobj = aau_mat.shape[0]
    print(objname,nobj)
    laplacexvec = []
    laplaceyvec = []
    laplaceBvec = []
    ...
    dfd.to_csv('b010_laplaceplane_'+objname+'_clones_'+yrsstr+'.csv',index=None)
    print(iname+1,nnames,objname,'end')