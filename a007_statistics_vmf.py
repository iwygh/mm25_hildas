...
for icon in range(ncons):
    ...
    dfl = pd.read_csv('b010_laplaceplane_'+output_label+'_'+yrsstr+'.csv')
...
dtyrs_str = '0.2yr'
yrsstr = tyrs_str + '_' + tstepyrs_str + '_' + dtyrs_str + '_jd' + str(jd)
tyrsvec_long = np.loadtxt('b003_tyrsvec_long_' + yrsstr + '.csv',delimiter=',')
nt_long = len(tyrsvec_long)
seed_asteroids = ['Potomac','Hilda','Schubart','Ismene']
labels = ['P','H','S','BFG']
nseeds = len(seed_asteroids)
aau_mat_planets_long = np.loadtxt('b003_planets_aau_long_'+yrsstr+'.csv',delimiter=',')
xinvar_vec_long = np.loadtxt('b003_planets_xinvar_long_'+yrsstr+'.csv',delimiter=',')
yinvar_vec_long = np.loadtxt('b003_planets_yinvar_long_'+yrsstr+'.csv',delimiter=',')
idegplanets_long = np.loadtxt('b003_planets_ideg_long_'+yrsstr+'.csv',delimiter=',')
Wdegplanets_long = np.loadtxt('b003_planets_nodedeg_long_'+yrsstr+'.csv',delimiter=',')
sini_long = np.sin(np.radians(idegplanets_long))
cosW_long = np.cos(np.radians(Wdegplanets_long))
sinW_long = np.sin(np.radians(Wdegplanets_long))
qplanets_long = sini_long * cosW_long
pplanets_long = sini_long * sinW_long
xJ_vec_long = qplanets_long[0,:]
yJ_vec_long = pplanets_long[0,:]
for iseed in range(nseeds):
    seed = seed_asteroids[iseed]
    output_label = labels[iseed]
    ideg_mat = np.loadtxt('b005_'+seed+'_ideg_clones_longandfew_'+yrsstr+'.csv',delimiter=',')
    Wdeg_mat = np.loadtxt('b005_'+seed+'_nodedeg_clones_longandfew_'+yrsstr+'.csv',delimiter=',')
    ...
    ymat = np.sin(irad_mat)*np.sin(Wrad_mat)
    zmat = np.cos(irad_mat)
    dfl = pd.read_csv('b010_laplaceplane_'+seed+'_clones_longandfew_'+yrsstr+'.csv')
    dflx_vec = dfl['laplacex'].to_list()
    dfly_vec = dfl['laplacey'].to_list()
    xcc_vec = []
    ycc_vec = []
    ...
    for it in range(nt_long):
        ...
        pval = acstats.rayleightest(np.radians(dWdeg))
        p_xcc_vec.append(pval)
        dx = xvec - xJ_vec_long[it]
        dy = yvec - yJ_vec_long[it]
        dideg = np.degrees(np.arcsin(np.sqrt(dx**2+dy**2)))
        dsini = np.sin(np.radians(dideg))
        dWdeg = np.degrees(np.arctan2(dy/dsini,dx/dsini))
        pval = acstats.rayleightest(np.radians(dWdeg))
        p_xJ_vec.append(pval)
        dx = xvec - xinvar_vec_long[it]
        dy = yvec - yinvar_vec_long[it]
        dideg = np.degrees(np.arcsin(np.sqrt(dx**2+dy**2)))
        dsini = np.sin(np.radians(dideg))
        dWdeg = np.degrees(np.arctan2(dy/dsini,dx/dsini))
        pval = acstats.rayleightest(np.radians(dWdeg))
    ...
    dfd = pd.DataFrame.from_dict(dictionary)
    dfd.to_csv('b007_'+seed+'_clones_longandfew_statistics_vmf_'+yrsstr+'.csv',index=None)
    plt.plot(tyrsvec_long/1e6,p_laplace_vec)
    plt.axhline(y=0.05,color='red')
    print(seed + ' clones longandfew laplace')
    print(np.sum(np.array(p_laplace_vec)<0.05)/len(p_laplace_vec))
    print('')
    plt.title(seed+' clones longandfew pval W uniform wrt laplace')
    plt.show()
    plt.plot(tyrsvec_long/1e6,p_xJ_vec)
    plt.axhline(y=0.05,color='red')
    print(seed + ' clones longandfew xJ')
    print(np.sum(np.array(p_xJ_vec)<0.05)/len(p_xJ_vec))
    print('')
    plt.title(seed+' clones longandfew pval W uniform wrt xJ')
    plt.show()
    plt.plot(tyrsvec_long/1e6,p_xcc_vec)
    plt.axhline(y=0.05,color='red')
    print(seed + ' clones longandfew xcc')
    print(np.sum(np.array(p_xcc_vec)<0.05)/len(p_xcc_vec))
    print('')
    plt.title(seed+' clones longandfew pval W uniform wrt xcc')
    plt.show()
    plt.plot(tyrsvec_long/1e6,p_xI_vec)
    plt.axhline(y=0.05,color='red')
    print(seed + ' clones longandfew xinvar')
    print(np.sum(np.array(p_xI_vec)<0.05)/len(p_xI_vec))
    print('')
    plt.title(seed+' clones longandfew pval W uniform wrt xinvar')
    plt.show()
    plt.scatter(xmat[:,-1],ymat[:,-1])
    plt.title(seed + ' clones longandfew xy')
    plt.show()