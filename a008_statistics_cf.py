...
for iseed in range(nseeds):
    ...
    dfd.to_csv('b008_'+seed+'_clones_statistics_cf_'+yrsstr+'.csv',index=None)
#%%
output_labels = ['P','H','S']
...
for icon in range(ncons):
    ...
    irad_mat = np.radians(ideg_mat_astorb[astorb_indices,:])
    Wrad_mat = np.radians(Wdeg_mat_astorb[astorb_indices,:])
    xmat = np.sin(irad_mat)*np.cos(Wrad_mat)
    ymat = np.sin(irad_mat)*np.sin(Wrad_mat)
    zmat = np.cos(irad_mat)
    xccvec = []
    ...
    dictionary = {'xcc':xccvec,\
                  ...
                  'sigs_997_deg':ss3vec}
    dfd = pd.DataFrame.from_dict(dictionary)
    dfd.to_csv('b008_'+seed+'_observed_statistics_cf_'+yrsstr+'.csv',index=None)
