...
# objects should be a dataframe that contains 'H_mag' and 'a_au' columns
# objects = pd.read_csv('a002_sample_data.csv')
objects = pd.read_csv('b001_astorb_horizons.csv')
...
fiducial_error = (objects['H_mag'].max() - objects['H_mag'].min())/2
...
_H  = objects['H_mag'].max() - objects['H_mag'].min()
_au = objects['a_au'].max() - objects['a_au'].min()
au_bins = pd.cut(objects['a_au'], int(_au/hexbin_x_size))

au_groups = objects.groupby(au_bins)
for interval, au_group in au_groups:
    if len(au_group) == 0:
        continue
    H_bins   = pd.cut(au_group['H_mag'], int(_H*2))
    H_groups = au_group.groupby(H_bins)

    most_pop_H = H_groups.count()['H_mag'].idxmax().mid
    nH         = sum(H_groups.count()['H_mag'])
    au         = interval.mid
...
for name, (fit_min, fit_max) in regions.items():
    print(name)
    region_objects = objects[ (objects['a_au'] >= fit_min) & (objects['a_au'] <= fit_max)]
    ...
    # region.
    region_errors = np.array((region_objects['H_mag'].max() - region_objects['H_mag'].min()) / np.sqrt(nH_per_bin))
    ...
    Hlim_for_region = completion_limit_model(np.median(region_objects['a_au']), np.median(samples))
    Hlim_for_region_error = completion_limit_model(np.median(region_objects['a_au']), samples).std()
