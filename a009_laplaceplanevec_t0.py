...
avec = np.linspace(start=3.8,stop=4.1,num=1001,endpoint=True)
na = len(avec)

Wmat_planets = np.loadtxt('b003_planets_nodedeg_'+yrsstr+'.csv',delimiter=',')
wmat_planets = np.loadtxt('b003_planets_perideg_'+yrsstr+'.csv',delimiter=',')
Mmat_planets = np.loadtxt('b003_planets_Mdeg_'+yrsstr+'.csv',delimiter=',')
imat_planets = np.loadtxt('b003_planets_ideg_'+yrsstr+'.csv',delimiter=',')
emat_planets = np.loadtxt('b003_planets_e_'+yrsstr+'.csv',delimiter=',')
amat_planets = np.loadtxt('b003_planets_aau_'+yrsstr+'.csv',delimiter=',')
Wmat_planets = Wmat_planets[:,0]
wmat_planets = wmat_planets[:,0]
Mmat_planets = Mmat_planets[:,0]
imat_planets = imat_planets[:,0]
emat_planets = emat_planets[:,0]
amat_planets = amat_planets[:,0]
...
ij = imat_planets[0]
Wj = Wmat_planets[0]
qj = np.sin(np.radians(ij))*np.cos(np.radians(Wj))
pj = np.sin(np.radians(ij))*np.sin(np.radians(Wj))
for ia in range(na):
    a_sample = avec[ia]
    q0,p0,i0,W0,B_scalar = laplace_plane_heliocentric_outerplanetsonly(a_sample,wmat_planets,\
        Wmat_planets,Mmat_planets,imat_planets,emat_planets,amat_planets)
    i0 = np.degrees(i0)
    W0 = np.degrees(W0)
    i_array[ia] = i0
    W_array[ia] = W0
...
dfd.to_csv('b009_laplaceplane_t0_'+yrsstr+'.csv',index=None)